...
具体地，"momentum"的大小定义为$$f_{ij}=\bf{\omega^{(k)}}\cdot\bf{X_{ij}^{(k)}}$$
其中：
\begin{enumerate}
    \item $f_{ij}$代表第i场比赛（按照表格顺序）中第j个point number之前的"momentum"
    \item $\bf{X_{ij}^{k}}$是一个k维的列向量，代表相对应时刻的一些影响因素，后面我们会具体给出
    \item $\omega^{k}$是一个k维的行向量，指影响因素具体的权重，我们会通过层次分析法来得到
    \item 这里的公式中，具体会分为两个不同的计算方法，一个分别代表自己发球的场次和对手发球的场次，
    我们可以写为$$\bf{\omega^{(k)}}=\bf{\omega_0^{(k)}}.\ast {\delta^{(k)}}$$
    代表两个相同维数向量对应位置相乘构成的向量，其中$\delta^{(k)}$为一个0,1向量，
    取决于是否是自己的发球轮次，在具体的计算中，我们会分两种情况带入具体考虑。\\
\end{enumerate}
\subsection{影响因素分析}

对于$\bf{X_{ij}^{k}}$的具体定义，我们认为，除了是否为发球方，还有许多其他因素
会造成影响，包括选手的技术，疲惫程度，以及比赛的实时心态。
基于以上三个大点，我们整理了九个因素来作为初步的影响因素，具体如下：
（画个好看的表）
